...
def build_line_item() -> LineItem:
    """Given a service code and quantity, return a LineItem with the amount."""
    def inner(code: str, quantity: int) -> LineItem:
        price = pricing.get_price(code)
        amount = round(price * quantity, 2)
        return LineItem(code=code, quantity=quantity, amount=amount)
